...
import {IPairFactory, IPair} from "../interfaces/adapters/IVelodrome.sol";

contract VelodromeAdapter is StyxAdapter {
    ...
    function getQuoteAndPair(
        ...
        if (IPairFactory(FACTORY).isPair(pairVolatile)) {
            amountVolatile = IPair(pairVolatile).getAmountOut(
                _amountIn,
                _tokenIn
            );
        }
