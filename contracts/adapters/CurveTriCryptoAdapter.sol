...
import {ICurvePool256} from "../interfaces/adapters/ICurvePool256.sol";
import {StyxAdapter} from "../shared/StyxAdapter.sol";

contract CurvePlainV2Adapter is StyxAdapter {
    ...
    function _getCoinByIndexSafe(
        address _pool,
        uint256 _index
    ) internal view returns (address token) {
        try ICurvePool256(_pool).coins(_index) returns (address _token) {
            token = _token;
        } catch {}
    }
