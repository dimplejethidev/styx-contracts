...
import {IGmxVault, IGmxVaultPriceFeed} from "../interfaces/adapters/IGmxVault.sol";
import {StyxAdapter} from "../shared/StyxAdapter.sol";

contract GmxAdapter is StyxAdapter {
    ...
    function _setVaultTkns(address _vault) internal {
        uint256 whitelistedTknsLen = IGmxVault(_vault)
    ...
    function _query(
        ...
        if (_validArgs(_amountIn, _tokenIn, _tokenOut))
            return _getAmountOut(_amountIn, _tokenIn, _tokenOut);
    }
    ...
    function _getAmountOut(
        ...
        (uint256 amountOut, uint256 usdgAmount) = _getGrossAmountOutAndUsdg(
            _amountIn,
            _tokenIn,
            _tokenOut
        );
    ...
    function _calcNetAmountOut(
        ...
        bool withinVaultLimits = _isWithinVaultLimits(
            _tokenIn,
            _tokenOut,
            _usdgAmount,
            netAmountOut
    ...
    function _amountOutAfterFees(
        uint256 _amountOut,
        uint256 _feeBasisPoints
    ) internal pure returns (uint256) {
        return
            (_amountOut * (BASIS_POINTS_DIVISOR - _feeBasisPoints)) /
    ...
    function reservedAmountExceeded(
        uint256 _newPoolBalTknOut,
        address _tokenOut
    ) internal view returns (bool) {
    ...
    function _getFeeBasisPoint(
        ...
        if (USE_VAULT_UTILS)
            return
                IGmxVault(VAULT).vaultUtils().getSwapFeeBasisPoints(
                    _tokenIn,
                    _tokenOut,
                    usdgAmount
                );
    ...
    function _calcFeeBasisPoints(
        ...
        bool isStableSwap = IGmxVault(VAULT).stableTokens(_tokenIn) &&
            IGmxVault(VAULT).stableTokens(_tokenOut);
        ...
        uint256 taxBps = isStableSwap
            ? IGmxVault(VAULT).stableTaxBasisPoints()
            : IGmxVault(VAULT).taxBasisPoints();
        uint256 feesBasisPoints0 = IGmxVault(VAULT).getFeeBasisPoints(
            _tokenIn,
            usdgAmount,
            baseBps,
            taxBps,
        ...
        uint256 feesBasisPoints1 = IGmxVault(VAULT).getFeeBasisPoints(
            _tokenOut,
            usdgAmount,
            baseBps,
            taxBps,
